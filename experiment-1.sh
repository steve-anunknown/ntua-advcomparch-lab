...
OUT_ROOT="/home/stefanos/Documents/programming-projects/uni/adv-comp-arch/lab1/parsec-3.0/parsec_workspace/outputs/exp1"

L1_FLAGS=(
"-L1c 32 -L1a 4 -L1b 64"
"-L1c 32 -L1a 8 -L1b 32"
"-L1c 32 -L1a 8 -L1b 64"
"-L1c 32 -L1a 8 -L1b 128"
"-L1c 64 -L1a 4 -L1b 64"
"-L1c 64 -L1a 8 -L1b 32"
"-L1c 64 -L1a 8 -L1b 64"
"-L1c 64 -L1a 8 -L1b 128" 
"-L1c 128 -L1a 8 -L1b 32"
"-L1c 128 -L1a 8 -L1b 64"
"-L1c 128 -L1a 8 -L1b 128" 
)
L2_FLAGS="-L2c 1024 -L2a 8 -L2b 128"
TLB_FLAGS="-TLBe 64 -TLBa 4 -TLBp 4096"

# the following should normally be executed but
# the executables are missing ...
# "${EXE_ROOT}/facesim -timing -threads 1"
# "${EXE_ROOT}/ferret ${INP_ROOT}/corel lsh ${INP_ROOT}/queries 10 20 1 output.txt"
...
# "${OUT_ROOT}/exp1-facesim"
# "${OUT_ROOT}/exp1-ferret"
OUTPUT_FILE=(
    "${OUT_ROOT}/blacksholes/exp1-blackscholes"
    "${OUT_ROOT}/bodytrack/exp1-bodytrack"
    "${OUT_ROOT}/canneal/exp1-canneal"
    "${OUT_ROOT}/fluidanimate/exp1-fluidanimate"
    "${OUT_ROOT}/freqmine/exp1-freqmine"
    "${OUT_ROOT}/rtview/exp1-rtview"
    "${OUT_ROOT}/streamcluster/exp1-streamcluster"
    "${OUT_ROOT}/swaptions/exp1-swaptions"
)

export LD_LIBRARY_PATH=~/Documents/programming-projects/uni/adv-comp-arch/lab1/parsec-3.0/pkgs/libs/hooks/inst/amd64-linux.gcc-serial/lib

for i in "${!L1_FLAGS[@]}";
do
    for j in "${!COMMANDS[@]}";
    do
        ${PIN_ROOT} -t ${SIM_ROOT} -o "${OUTPUT_FILE[$j]}${i}.txt" ${L1_FLAGS[$i]} ${L2_FLAGS} ${TLB_FLAGS} -- ${COMMANDS[$j]}
        > /var/log/syslog
    done
done
